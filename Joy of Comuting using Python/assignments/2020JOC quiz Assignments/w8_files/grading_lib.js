...
BaseQuestion.prototype.displayFeedback = function(feedback, qt_feedback_visibility = true) {
...
McQuestion.prototype.setStudentAnswer = function(answer) {
  ...
  if (answer instanceof Array || (answer.hasOwnProperty("responses") && ! answer.hasOwnProperty("permutation"))) {
    if(answer.hasOwnProperty("responses")){
...
SaQuestion.prototype.getStudentAnswer = function() {
  ...
  return {'response': response, 'is_answered': is_answered};
};
...
QuestionGroup.prototype.init = function() {
  ...
  if(this.el.hasClass('qt-embedded-question-group')){
    this.el.find('div[id^="sa_question_"]').each(function(index, element){
        $(element).parent().find('div.qt-response > input').css({"border": "none", "border-bottom": "1px solid black", "outline": "none"})
        replaceQuestionString(element);
    })
    this.el.find('div[id^="nm_question_"]').each(function(index, element){
        replaceQuestionString(element);
    })
    this.el.append('<div class="qt-points">'+ total_points +' points </div>');
  }
};

function replaceQuestionString(element){
    var question_string = $(element).text()
    $(element).parent().find('div.qt-response').addClass("gcb-inline-div")
    var input_value = $(element).parent().find('div.qt-response > input').val()
    replacement_string = '<div class="qt-response gcb-inline-div">' + $(element).parent().find('div.qt-response').html() + '</div>'
    $(element).parent().find('div.qt-response').remove()
    question_string = question_string.replace("[[SHORT_ANSWER]]", replacement_string)
    $(element).html(question_string);
    $(element).find('div.qt-response > input').val(input_value)
}
...
QuestionGroup.prototype.displayFeedback = function(feedback, qt_feedback_visibility=true) {
  var that = this;
...
QuestionGroup.prototype.getStudentAnswer = function() {
  ...
  $.each(this.questions, function(index, question) {
    ...
    if(state[question.id]['is_answered']){
...
function gradeScoredLesson(questions, messages, question_batch_id) {
  ...
  $.each(questions, function(idx, question) {
    ...
    if(window.MathJax && $('div.qt-grade-scored-lesson').children('button.qt-check-answer-button')[0]) {
        question.displayFeedback(grade.feedback, qt_feedback_visibility=false);
    }
    else {
        question.displayFeedback(grade.feedback, qt_feedback_visibility=true);
    }
  ...
  $('span.score').text(score + '/' + totalWeight+'='+Math.round((score/totalWeight)*100)+'%');
...
function gradeAssessment(questions, unitId, xsrfToken, graderUri) {
  ...
  $.each(questions, function(idx, question) {
    ...
    if(answers[question.id]['is_answered']){
     allow_submission = true
    }
  });

  if(allow_submission){
    submitForm(graderUri || '', {
    ...
    alert("Atleast one question is required to be answered to proceed for submission.")
  }
